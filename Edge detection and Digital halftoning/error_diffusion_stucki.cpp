    ...
    for (int i = 2; i < height + 2; i++)
    {
        if ((i - 2) % 2 != 0)
        {
            for (int j = 2; j < width + 2; j++)
            {
                ...
                float err = (extendedImage[i][j] - halftonedImage[i - 2][j - 2])/42.0;
                extendedImage[i][j + 1] += 8 * err;
                extendedImage[i][j + 2] += 4 * err;

                extendedImage[i + 1][j - 2] += 2 * err;
                extendedImage[i + 1][j - 1] += 4 * err;
                extendedImage[i + 1][j] += 8 * err;
                extendedImage[i + 1][j + 1] += 4*err;
                extendedImage[i + 1][j + 2] += 2*err;

                extendedImage[i + 2][j - 2] += 1 * err;
                extendedImage[i + 2][j - 1] += 2 * err;
                extendedImage[i + 2][j] += 4 * err;
                extendedImage[i + 2][j + 1] += 2*err;
